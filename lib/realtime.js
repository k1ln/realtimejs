                ...
                if (typeof target[key] === 'object' && target[key] !== null && target[key].function == undefined) {
                    ...
                    if (target[key] !== undefined || target[newkey] !== undefined) {
                        if (key.substr(0, 1) == "!") {
                            ...
                            try{
                                return target[keyarr[2]].arrHistory[arrHistory.length-keyarr[2]*1];
                            } catch {
                                return undefined;
                            }
            ...
            set(target, key, value) {
                ...
                if (typeof value == 'string') {
                    ...
                    if (value2.substr(0, 2) == "$:") {
                        ...
                        if(key.substr(0, 1) !== "#")
                        {
                            /*if (typeof target["#old#"+key] !== 'object') {
                                target["#old#"+key] = {};
                            }

                            target["#old#"+key].value = target[key].value;
                            target["#old#"+key].function = () => {
                                return target["#old#"+key].value;
                            }
                            */
                            
                            if(target[key].history)
                            {
                                if(target[key].arrHistory == undefined){
                                    target[key].arrHistory = [];
                                }
                                var obj = {};
                                obj.value = target[key].value;
                                target[key].arrHistory.push(obj); 
                            } 
...
class Xvar {
    ...
    static setInnerHTML(elm, html, props) {
        ...
        Array.from(elm.querySelectorAll("script")).forEach(oldScript => {
            ...
            if(props){
                script = script.replaceAll("_props",props)
            }
